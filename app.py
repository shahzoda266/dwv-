...
details:hover { border-color: #9b1c2e !important; }
details[open] {
    border-color: #9b1c2e !important;
    box-shadow: 0 0 0 1px #9b1c2e15, 0 4px 16px #9b1c2e0a !important;
}
details summary {
...
details summary:hover { color: #9b1c2e !important; }
...
.stTextInput input:focus, .stNumberInput input:focus, .stTextArea textarea:focus {
    border-color: #9b1c2e !important;
    box-shadow: 0 0 0 2px rgba(123,29,46,0.10) !important;
}
...
/* Slider */
.stSlider > div > div > div > div { background: #1f1f1f !important; }

/* Dataframe */
.stDataFrame { border: 1px solid #e0e0e0 !important; border-radius: 10px !important; overflow: hidden !important; }
thead tr th {
...
[data-testid="stFileUploader"]:hover { border-color: #9b1c2e !important; }
...
h1, h2, h3, h4 { font-family: 'Space Mono', monospace !important; color: #111111 !important; }
</style>
...
<div style='padding:24px 0 8px 0'>
    <p style='font-family:Space Mono,monospace;font-size:11px;color:#888888;letter-spacing:3px;text-transform:uppercase;margin:0 0 16px 0'>Navigate</p>
</div>
""", unsafe_allow_html=True)

page = st.sidebar.selectbox("", ["Upload & Overview", "Cleaning & Preparation"], label_visibility="collapsed")

st.sidebar.markdown("<hr style='border-color:#e0e0e0;margin:16px 0'>", unsafe_allow_html=True)

if "df" in st.session_state:
    _df = st.session_state.df
    st.sidebar.markdown(f"""
    <p style='font-family:Space Mono,monospace;font-size:10px;color:#888888;letter-spacing:2px;text-transform:uppercase;margin:0 0 12px 0'>Dataset</p>
    <div style='display:flex;flex-direction:column;gap:8px'>
        <div style='background:#ffffff;border-radius:8px;padding:10px 14px;border:1px solid #e0e0e0;border-left:3px solid #9b1c2e'>
            <span style='color:#888888;font-size:10px;text-transform:uppercase;letter-spacing:1px'>Rows</span><br>
            <span style='color:#1f1f1f;font-family:Space Mono,monospace;font-size:18px;font-weight:700'>{_df.shape[0]:,}</span>
        </div>
        <div style='background:#ffffff;border-radius:8px;padding:10px 14px;border:1px solid #e0e0e0;border-left:3px solid #9b1c2e'>
            <span style='color:#888888;font-size:10px;text-transform:uppercase;letter-spacing:1px'>Columns</span><br>
            <span style='color:#1f1f1f;font-family:Space Mono,monospace;font-size:18px;font-weight:700'>{_df.shape[1]}</span>
        </div>
        <div style='background:#ffffff;border-radius:8px;padding:10px 14px;border:1px solid #e0e0e0;border-left:3px solid #c0392b'>
            <span style='color:#888888;font-size:10px;text-transform:uppercase;letter-spacing:1px'>Missing</span><br>
            <span style='color:#c0392b;font-family:Space Mono,monospace;font-size:18px;font-weight:700'>{_df.isnull().sum().sum():,}</span>
        </div>
        <div style='background:#ffffff;border-radius:8px;padding:10px 14px;border:1px solid #e0e0e0;border-left:3px solid #d97706'>
            <span style='color:#888888;font-size:10px;text-transform:uppercase;letter-spacing:1px'>Duplicates</span><br>
            <span style='color:#d97706;font-family:Space Mono,monospace;font-size:18px;font-weight:700'>{_df.duplicated().sum():,}</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
<div style='margin-bottom:32px'>
    <h1 style='font-family:Space Mono,monospace;font-size:34px;font-weight:700;color:#111111;margin:0;line-height:1.1'>
        Data Wrangler <span style='color:#9b1c2e'>&</span> Visualizer
    </h1>
    <p style='color:#888888;font-size:17px;margin:8px 0 0 0'>Clean, transform, and visualize your data effortlessly</p>
</div>
""", unsafe_allow_html=True)
...
if page == "Upload & Overview":
    ...
    for i, (icon, title, sub) in enumerate(features):
        with cols_feat[i]:
            st.markdown(f"""
            <div style='background:#e8e8e8;border:1px solid #cccccc;border-radius:12px;padding:16px 10px;text-align:center;margin-bottom:24px'>
                <div style='font-size:22px;margin-bottom:6px'>{icon}</div>
                <div style='font-family:Space Mono,monospace;font-size:12px;font-weight:700;color:#111111'>{title}</div>
                <div style='font-size:12px;color:#555555;margin-top:4px'>{sub}</div>
            </div>
            """, unsafe_allow_html=True)

    st.markdown("""
    <p style='font-family:Space Mono,monospace;font-size:11px;color:#9b1c2e;letter-spacing:2px;text-transform:uppercase;margin:0 0 8px 0'>Step 01</p>
    <h2 style='font-family:Space Mono,monospace;font-size:20px;color:#111111;margin:0 0 16px 0'>Upload Your Dataset</h2>
    """, unsafe_allow_html=True)
    ...
    if "df" not in st.session_state:
        ...
        st.markdown("""
        <p style='font-family:Space Mono,monospace;font-size:11px;color:#9b1c2e;letter-spacing:2px;text-transform:uppercase;margin:0 0 8px 0'>Step 02</p>
        <h2 style='font-family:Space Mono,monospace;font-size:20px;color:#111111;margin:0 0 16px 0'>Dataset Overview</h2>
        """, unsafe_allow_html=True)
        ...
        st.markdown("""
        <p style='font-family:Space Mono,monospace;font-size:11px;color:#9b1c2e;letter-spacing:2px;text-transform:uppercase;margin:16px 0 8px 0'>Step 03</p>
        <h2 style='font-family:Space Mono,monospace;font-size:20px;color:#111111;margin:0 0 16px 0'>Summary Statistics</h2>
        """, unsafe_allow_html=True)
        ...
        with st.expander("Duplicates", expanded=False):
            st.markdown(f"""
            <div style='background:#fafafa;border-radius:10px;padding:20px;text-align:center;border:1px solid #e0e0e0'>
                <p style='color:#888888;font-size:11px;text-transform:uppercase;letter-spacing:1.5px;margin:0 0 4px 0'>Duplicate Rows Found</p>
                <p style='color:#1f1f1f;font-family:Space Mono,monospace;font-size:36px;font-weight:700;margin:0'>{df.duplicated().sum():,}</p>
            </div>
            """, unsafe_allow_html=True)
...
# ================= PAGE B =================
    ...
    if "df" not in st.session_state:
        ...
        st.markdown("""
        <p style='font-family:Space Mono,monospace;font-size:11px;color:#9b1c2e;letter-spacing:2px;text-transform:uppercase;margin:0 0 8px 0'>Page B</p>
        <h2 style='font-family:Space Mono,monospace;font-size:22px;color:#111111;margin:0 0 4px 0'>Cleaning Tools</h2>
        <p style='color:#888888;font-size:13px;margin:0 0 24px 0'>Click any section below to expand it and apply transformations.</p>
        """, unsafe_allow_html=True)
        ...
        with st.expander("Outlier Detection & Handling", expanded=False):
            if len(num_cols) > 0:
                ...
                st.markdown(f"""
                <div style='background:#fafafa;border:1px solid #e0e0e0;border-radius:10px;padding:16px;margin-bottom:12px;display:flex;gap:24px'>
                    <div><span style='color:#888888;font-size:11px;text-transform:uppercase;letter-spacing:1px'>Outliers</span><br>
                    <span style='color:#c0392b;font-family:Space Mono,monospace;font-size:22px;font-weight:700'>{len(outliers):,}</span></div>
                    <div><span style='color:#888888;font-size:11px;text-transform:uppercase;letter-spacing:1px'>Lower Bound</span><br>
                    <span style='color:#1f1f1f;font-family:Space Mono,monospace;font-size:22px;font-weight:700'>{lower:.2f}</span></div>
                    <div><span style='color:#888888;font-size:11px;text-transform:uppercase;letter-spacing:1px'>Upper Bound</span><br>
                    <span style='color:#1f1f1f;font-family:Space Mono,monospace;font-size:22px;font-weight:700'>{upper:.2f}</span></div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with st.expander("Column Operations", expanded=False):
            ...
            st.markdown("<hr style='border-color:#e0e0e0;margin:20px 0'>", unsafe_allow_html=True)
